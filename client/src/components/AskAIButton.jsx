...
export default function AskAIButton({ 
    ...
    if (variant === 'chip') {
        return (
            ...
            >
                ...
                <span className="text-indigo-700 dark:text-indigo-300 font-semibold whitespace-nowrap" style={{ WebkitTextFillColor: 'currentColor' }}>{label}</span>
            </button>
        );
    }
    ...
    if (variant === 'button') {
        return (
            <button
                ...
                onClick={handleClick}
                style={{ WebkitTextFillColor: 'currentColor' }}
                className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-xl text-xs font-semibold bg-gradient-to-r from-indigo-600 to-indigo-500 text-white shadow-md hover:shadow-indigo-500/25 hover:from-indigo-500 hover:to-indigo-400 transition-all cursor-pointer flex-shrink-0 ${className}`}
                title={`Ask AI: ${prompt || label}`}
            >
        ...
        return (
            ...
            >
                <div className="flex items-center gap-2 truncate">
                    ...
                    <span className="truncate font-medium text-slate-700 dark:text-slate-200" style={{ WebkitTextFillColor: 'currentColor' }}>{label}</span>
                </div>
    ...
    return (
        <button
            ...
            onClick={handleClick}
            style={{ WebkitTextFillColor: 'currentColor' }}
            className={`p-1.5 rounded-lg text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 dark:text-slate-400 dark:hover:text-indigo-400 dark:hover:bg-indigo-500/10 border border-transparent hover:border-indigo-200 dark:hover:border-indigo-500/20 transition-all cursor-pointer flex-shrink-0 ${className}`}
            title={`Ask AI: ${prompt || label}`}
        >
