    ...
    return (
        <div className="space-y-6">
            <div>
                ...
                <p className="text-text-muted mt-1">Real-time competitor price monitoring</p>
            </div>
            ...
            <div className="glass-card p-6">
                <div className="flex items-center justify-between mb-4">
                    <h2 className="text-lg font-semibold text-text flex items-center gap-2">
                        <HiOutlineScale className="w-5 h-5 text-primary" /> Price History
                    </h2>
                    ...
                    <ResponsiveContainer width="100%" height={300}>
                        <LineChart data={history}>
                            ...
                            {Object.keys(competitorColors).map(comp => (
                                history.some(h => h[comp]) && <Line key={comp} type="monotone" dataKey={comp} stroke={competitorColors[comp]} strokeWidth={2} dot={{ r: 3 }} />
                            ))}
                        </LineChart>
                    </ResponsiveContainer>
                ) : (
                    <p className="text-text-muted text-center py-12">Select a product to view competitor price history</p>
                )}
            </div>
            ...
            <div className="glass-card overflow-hidden">
                <div className="px-6 py-4 border-b border-border/50">
                    <h2 className="text-lg font-semibold text-text">Latest Competitor Prices</h2>
                </div>
                <div className="overflow-x-auto">
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-border/50">
                                <th className="text-left px-6 py-3 text-sm font-medium text-text-muted">Product</th>
                                <th className="text-right px-6 py-3 text-sm font-medium text-text-muted">Our Price</th>
                                <th className="text-left px-6 py-3 text-sm font-medium text-text-muted">Competitor</th>
                                <th className="text-right px-6 py-3 text-sm font-medium text-text-muted">Their Price</th>
                                <th className="text-center px-6 py-3 text-sm font-medium text-text-muted">Stock</th>
                                <th className="text-right px-6 py-3 text-sm font-medium text-text-muted">Diff</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-border/30">
                            {Object.entries(productPrices).map(([pid, data]) =>
                                data.competitors.map((comp, i) => {
                                    ...
                                    return (
                                        <tr key={`${pid}-${i}`} className="hover:bg-surface-lighter/30 transition-colors">
                                            {i === 0 && (
                                                <>
                                            ...
                                            <td className="px-6 py-3 text-center">
                                                <span className={comp.inStock ? 'badge-success' : 'badge-danger'}>{comp.inStock ? 'In Stock' : 'OOS'}</span>
                                            </td>
                                            <td className="px-6 py-3 text-right">
                                                <span className={`text-sm font-medium flex items-center justify-end gap-1 ${diff > 0 ? 'text-success' : 'text-danger'}`}>
